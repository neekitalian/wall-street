(() => {
  ...
  function renderEntry() {
    ...
    document.getElementById("multiplayerContent").innerHTML = `<div class="multi-intro"><p>${t("Create an anonymous room or join with a six-character code. One player occupies each role.", "创建匿名房间，或使用六位房间码加入。每个角色只能由一名玩家担任。")}</p></div><div class="role-picker-preview"><img id="selectedRoleFace" src="${avatar("founder")}" alt=""><div><span>${t("YOUR CHARACTER","你的角色")}</span><strong id="selectedRoleName">${roleCopy.founder[isZh()?1:0]}</strong></div></div><div class="multi-form"><label>${t("Display name","玩家名称")}<input id="multiName" maxlength="40" autocomplete="nickname"></label><label>${t("Role","角色")}<select id="multiRole">${Object.entries(roleCopy).map(([id,label]) => `<option value="${id}">${label[isZh()?1:0]}</option>`).join("")}</select></label><label>${t("Room code (to join)","房间码（加入时填写）")}<input id="multiCode" maxlength="6" autocomplete="off"></label></div><p id="multiError" class="multi-error"></p><div class="multi-buttons"><button id="createRoom" class="primary-action" type="button">${t("Create room","创建房间")}</button><button id="joinRoom" class="primary-action" type="button">${t("Join room","加入房间")}</button></div>`;
  ...
  function renderRoom() {
    ...
    const seats = Object.entries(roleCopy).map(([role,label]) => `<div class="seat ${playersByRole[role] ? "occupied" : ""}"><img src="${avatar(role)}" alt="${label[isZh()?1:0]}"><div><span>${label[isZh()?1:0]}</span><strong>${playersByRole[role]?.name || t("OPEN","空缺")}</strong>${active ? `<small>${submitted.has(role) ? t("LOCKED","已提交") : t("DECIDING","决策中")}</small>` : ""}</div></div>`).join("");
