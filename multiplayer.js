(() => {
  ...
  let session = null; let roomState = null; let pollTimer = null; let busy = false;
  ...
  async function enterRoom(mode) {
    ...
    try {
      const payload = await api(mode === "create" ? "/api/rooms-create" : "/api/rooms-join", { method:"POST", body:JSON.stringify({ name, role, code }) });
  ...
  function renderRoom() {
    ...
    const seats = Object.entries(roleCopy).map(([role,label]) => `<div class="seat ${playersByRole[role] ? "occupied" : ""}"><span>${label[isZh()?1:0]}</span><strong>${playersByRole[role]?.name || t("OPEN","空缺")}</strong>${active ? `<small>${submitted.has(role) ? t("LOCKED","已提交") : t("DECIDING","决策中")}</small>` : ""}</div>`).join("");
    ...
    let controls = "";
    if (roomState.status === "lobby") controls = session.player.is_host ? `<button id="startRoom" class="primary-action" type="button" ${roomState.players.length === 5 ? "" : "disabled"}>${t("Start when all five seats are filled","五个席位到齐后开始")}</button>` : `<p class="waiting-copy">${t("Waiting for the host to start.","等待房主开始游戏。")}</p>`;
    else if (active && !submitted.has(session.player.role)) controls = `<p class="role-prompt">${t("Your decision as","你当前的角色：")} ${roleCopy[session.player.role][isZh()?1:0]}</p><div class="role-actions">${actionCopy[session.player.role].map(([value,en,zh]) => `<button type="button" data-choice="${value}">${t(en,zh)}</button>`).join("")}</div>`;
